...
export function ArtFilter({
  ...
  function onChangeFilter(filterHistory: ChainedFilter[]): void {
    const filtered = [...artImages].filter((n) =>
      filterHistory.every((f) => (f.filterApply ? f.filterApply!(n) : true))
    )
    const sorted = filterHistory.findLast((h) => !!h.sortApply)
    if (sorted) {
      setFilteredImages(
        orderBy(filtered, sorted.sortApply!.key, sorted.sortApply?.option)
      )
    } else {

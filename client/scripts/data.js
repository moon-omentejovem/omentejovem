...
const client = createPublicClient({
  chain: mainnet,
  transport: http(
    'https://eth-mainnet.g.alchemy.com/v2/It_0ksmuyjkAvu061QgANvhk8OPUIzDx'
  )
})
...
async function getMintDate(contractAddress, tokenId) {
  try {
    const logs = await client.getLogs({
      ...
      fromBlock: BigInt(22592650 - 100),
      toBlock: BigInt(22592650 + 100)
    })

    if (logs.length === 0) {
      console.log(`No mint found for ${contractAddress}:${tokenId}`)
      return null
    }
    ...
    console.error(
      `Error processing ${contractAddress}:${tokenId}:`,
      error.message
    )
    return {
      contractAddress,
...
async function processAllNFTs() {
  ...
  for (const nft of ALL_NFTS) {
    // Skip Tezos NFTs (those starting with KT)
    if (nft.startsWith('KT')) {
      console.log(`Skipping Tezos NFT: ${nft}`)
      continue
    }

    const [contractAddress, tokenId] = nft.split(':')
    console.log(`Processing ${contractAddress}:${tokenId}...`)
    const result = await getMintDate(contractAddress, tokenId)
    results.push(result)

...
export async function fetchEditionNfts() {
  try {
    ...
    // Combine both arrays
    const nfts = [...ethereumNfts, ...tezosNfts]

    console.log('Processing NFTs...')

    // Order by mint date newest first
    nfts.sort((a, b) => {
      const aMintDate = a.contract.address.startsWith('KT')
        ? a.mint.timestamp
        : mintDates.find(
            (mint: MintDate | null) =>
              mint &&
      ...
      const bMintDate = b.contract.address.startsWith('KT')
        ? b.mint.timestamp
        : mintDates.find(
            (mint: MintDate | null) =>
              mint &&
    ...
    // Only return if contract.type === 'ERC1155' or FA2
    const filteredNfts = nfts.filter((nft) => {
      if (nft.contract.address.startsWith('KT')) {
        return nft.tokenType === 'FA2'
      }

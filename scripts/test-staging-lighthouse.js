...
async function createReportsDir(subDir) {
  const reportsDir = path.join(__dirname, '..', 'reports', 'lighthouse', subDir)
...
async function main() {
  console.log(`🔍 Testando ${STAGING_URL} em ${new Date().toISOString()}`)

  try {
    const outputDir = await createReportsDir('staging')

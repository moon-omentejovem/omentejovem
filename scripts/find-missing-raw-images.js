...
async function findAndFixMissingRawImages() {
  ...
  try {
    ...
    const { data: artworks, error: fetchError } = await supabase
      .from('artworks')
      .select('id, title, slug, image_url, raw_image_url, image_path, raw_image_path')
    ...
    console.log(`📊 Found ${artworks.length} artworks with missing raw_image_url`)
    ...
    for (const artwork of artworks) {
      ...
      if (bestMatch && bestConfidence >= 65) { // Confiança mínima de 65%
        foundMatches++
        console.log(`✅ Found match: ${bestMatch.fileInfo.file.name}`)
        console.log(`   Strategy: ${bestMatch.strategy}, Confidence: ${bestConfidence}%`)
        ...
        if (updateError) {
          console.error(`❌ Failed to update artwork ${artwork.id}:`, updateError)
        } else {
          updatedArtworks++
        ...
        if (bestMatch) {
          console.log(`   Best candidate: ${bestMatch.fileInfo.file.name} (${bestConfidence}% confidence)`)
        }
      }
    }

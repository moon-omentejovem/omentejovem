...
async function optimizeImageUrls() {
  ...
  try {
    ...
    for (const artwork of artworks) {
      ...
      if (
        artwork.image_url?.includes('i.seadn.io') &&
        !artwork.image_url.includes('w=500')
      ) {
        optimizedUrl =
          artwork.image_url +
          (artwork.image_url.includes('?') ? '&' : '?') +
          'w=500&auto=format'
        needsUpdate = true
      }
...
async function enhanceDescriptions() {
  ...
  try {
    ...
    console.log(
      `📊 Found ${artworks.length} artworks needing description enhancement`
    )

    // Default descriptions for some known artworks
    const defaultDescriptions = {
      'sitting-at-the-edge':
        'Part of the Stories on Circles collection, exploring moments of contemplation and transition.',
      'two-voices-one-circle':
        'A visual dialogue between different perspectives within a unified circular narrative.',
      'the-ground-was-my-teacher':
        'Reflecting on lessons learned from nature and our connection to the earth.',
      'i-had-dreams-about-you':
        'An intimate exploration of memory, dreams, and connection across time and space.',
      'mapping-the-unseen':
        'Visualizing the invisible connections and patterns that shape our understanding of reality.',
      'playing-chess-with-love':
        'A metaphorical game where strategy meets emotion in the pursuit of connection.',
      'all-time-high-discovery':
        'Capturing the euphoria of breakthrough moments and personal revelations.',
      'i-am-where-you-arent':
        'Exploring themes of presence, absence, and the spaces between connection and solitude.',
      'before-birth':
        'A meditation on potential, creation, and the moments before manifestation.',
      'he-left-as-a-dot':
        'The conclusion of a journey, reduced to its essential element - a single point of departure.'
    }
...
async function fixDataInconsistencies() {
  ...
  try {
    ...
    if (emptyTokenIds.length > 0) {
      console.log(
        `🔧 Found ${emptyTokenIds.length} artworks with empty token_id`
      )

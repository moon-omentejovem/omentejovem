...
function findMatchingRawFile(artworkTitle, optimizedUrl, rawFiles) {
  const cleanTitle = artworkTitle.toLowerCase()
    .replace(/[^a-z0-9]/g, '_')
    .replace(/_+/g, '_')
    .replace(/^_|_$/g, '')
  ...
  if (optimizedUrl) {
    ...
    match = rawFiles.find(file => {
      const cleanFileName = extractCleanName(file)
      return cleanFileName === cleanOptimizedName || 
...
async function processArtwork(artwork, rawFiles) {
  ...
  if (matchingFile) {
    ...
    if (newExists) {
      ...
      console.log(`   ⚠️ Arquivo encontrado mas também retorna 404: ${newRawUrl}`)
    }
  }
...
async function main() {
  ...
  try {
    ...
    console.log(`📊 Encontrados ${rawFiles.length} arquivos na pasta artworks/raw\n`)
    
    if (rawFiles.length === 0) {
      console.error('❌ Nenhum arquivo encontrado na pasta artworks/raw do bucket!')
      return
    }

...
async function main() {
  ...
  const files = fs.readdirSync(newSeriesDir).filter(file => file.endsWith('.jpg'));
  ...
  for (const file of files) {
    ...
    if (!slug) {
      console.log(`⚠️  Arquivo ${file} não possui mapeamento para slug, pulando...`);
      continue;
    }
    ...
    const uploadedRawPath = await uploadFileToSupabase(localFilePath, rawImagePath);
    ...
    const uploadedOptimizedPath = await uploadFileToSupabase(localFilePath, optimizedImagePath);
  ...
  if (errorCount === 0) {
    ...
    console.log('💡 Você pode agora executar o script de verificação novamente.');
  } else {

...
async function verifyDataIntegrity() {
  ...
  try {
    ...
    console.log(`🔗 Total series-artwork relationships: ${relationships.length}`)
    ...
    const { data: seriesWithCounts, error: countsError } = await supabase
      .from('series')
      .select(`
        name,
        slug,
...
async function cleanMigratedData() {
  ...
  try {
    ...
    if (!confirmClean) {
      console.log('⚠️  This will delete ALL artworks, series, and relationships!')
      console.log('⚠️  Add --confirm flag to proceed: node scripts/data-tools.js clean --confirm')
      return
    }
...
async function exportDataBackup() {
  ...
  try {
    ...
    console.log(`📊 Exported ${artworks.length} artworks, ${series.length} series, ${relationships.length} relationships`)
